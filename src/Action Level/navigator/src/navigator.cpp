...
Navigator::Navigator(const rclcpp::NodeOptions &options) : Node("navigator", options){
    ...
    nav_msg_sub_ = this->create_subscription<navigator_interfaces::msg::Navigate>(
        "to_navigator", 10, std::bind(&Navigator::nav_callback, this, std::placeholders::_1), subOpt);
    current_pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("/navigator/current_pose", 10, pubOpt);
